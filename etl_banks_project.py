...
def transform(df, csv_path):
    ...
    currencies_data = pd.read_csv(csv_path)

    currencies_dict = currencies_data.set_index('Currency').to_dict()['Rate']

    df['MC_GBP_Billion'] = [np.round(x * currencies_dict['GBP'], 2) for x in df['MC_USD_Billion']]
    df['MC_EUR_Billion'] = [np.round(x * currencies_dict['EUR'], 2) for x in df['MC_USD_Billion']]
    df['MC_INR_Billion'] = [np.round(x * currencies_dict['INR'], 2)for x in df['MC_USD_Billion']]
